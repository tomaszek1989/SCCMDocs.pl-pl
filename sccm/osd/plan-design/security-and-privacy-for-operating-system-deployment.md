---
title: "Bezpieczeństwo i prywatność wdrażania systemów operacyjnych | Dokumentacja firmy Microsoft"
description: "Informacje o zabezpieczeniach i prywatności najlepsze rozwiązania dotyczące wdrażania systemu operacyjnego w programie System Center Configuration Manager."
ms.custom: na
ms.date: 10/06/2016
...
Ten temat zawiera informacje o ochronie prywatności dotyczące wdrażania systemu operacyjnego w programie System Center Configuration Manager i zabezpieczeń.  

##  <a name="BKMK_Security_HardwareInventory"></a>Najlepsze rozwiązania dotyczące wdrażania systemu operacyjnego  
 Poniższe najlepsze rozwiązania należy użyć podczas wdrażania systemów operacyjnych za pomocą programu Configuration Manager:  
...
-   **Użyj zabezpieczonej lokalizacji, tworząc nośniki dla obrazów systemu operacyjnego**  
...
-   **Chronić pliki certyfikatu (pfx) przy użyciu silnego hasła i jeśli będą przechowywane w sieci, należy zabezpieczyć kanał sieci po ich zaimportowaniu do programu Configuration Manager**  
...
-   **Gdy dostawca programu SMS jest na komputerze lub komputerach innych niż serwer lokacji, należy zabezpieczyć kanał komunikacji w celu ochrony obrazów rozruchowych**  
...
-   **Włącz punkty dystrybucji dla komunikacji klienta obsługującego środowisko PXE tylko w chronionych segmentach sieci**  
...
-   **Skonfiguruj punkty dystrybucji obsługujące środowisko PXE odpowie na żądania środowiska PXE tylko na określonych interfejsach sieciowych**  
...
-   **Wymagaj hasła do rozruchu w środowisku PXE**  
     ...
     Wymagaj hasła do rozruchu w środowisku PXE, ta konfiguracja dodaje dodatkowy poziom zabezpieczeń do procesu rozruchu PXE, aby ułatwić ochronę przed nieautoryzowanymi klientami dołączającymi hierarchii programu Configuration Manager.  

-   **Nie należy uwzględniać aplikacji branżowych lub oprogramowania zawierającego poufne dane w obrazie, który będzie używany do rozruchu w środowisku PXE lub multiemisji**  
...
-   **Nie należy uwzględniać aplikacji branżowych lub oprogramowania zawierającego poufne dane w pakiety oprogramowania, które są instalowane przy użyciu zmiennych sekwencji zadań**  
...
-   **Korzystać z najnowszej wersji programu użytkownika migracji stanu narzędzie (USMT) obsługującego programu Configuration Manager**  
...
-   **Ręcznie usuń foldery z punku migracji stanu zlikwidowane**  

     Po usunięciu folderu punktu migracji stanu w konsoli programu Configuration Manager na właściwości punktu migracji stanu nie powoduje usunięcia folderu fizycznego. Aby zabezpieczyć dane migracji stanu użytkownika przed ujawnieniem informacji, należy ręcznie usunąć udział sieciowy i folder.  

-   **Nie należy konfigurować zasad usuwania, tak aby stan użytkownika był usuwany natychmiast**  
...
-   **Ręcznie Usuń skojarzenia komputera po przywróceniu stanu użytkownika migracji danych i zweryfikowaniu**  

     Menedżer konfiguracji automatycznie nie usuwa skojarzeń komputerów. Ochrona danych stanu użytkownika przez ręczne usunięcie skojarzeń komputera nie jest już wymagana.  

-   **Ręcznie utwórz kopię zapasową danych migracji stanu użytkownika w punkcie migracji stanu**  

     Kopia zapasowa programu Configuration Manager nie uwzględnia danych migracji stanu użytkownika.  

-   **Należy pamiętać o włączeniu funkcji BitLocker po zainstalowaniu systemu operacyjnego**  

     Jeśli komputer obsługuje funkcję BitLocker, musisz ją wyłączyć za pomocą kroku sekwencji zadań, jeśli chcesz przeprowadzić dyskretną instalację systemu operacyjnego. Configuration Manager nie włącza funkcji BitLocker po zainstalowaniu systemu operacyjnego, więc należy ręcznie włączyć funkcję BitLocker.  

-   **Wdrożenie kontroli dostępu w celu ochrony nośnika wstępnego**  
...
-   **Jeśli zachodzi potrzeba wdrożenia systemów operacyjnych na nieznanym komputerze, należy zaimplementować kontrolę dostępu, aby nieautoryzowane komputery z połączenia z siecią**  
...
-   **Włącz szyfrowanie pakietów multiemisji**  
     ...
     Dla każdego pakietu wdrożenia systemu operacyjnego masz możliwość włączenia szyfrowania transferowania Menedżera konfiguracji pakietu przy użyciu multiemisji. Ta konfiguracja uniemożliwia nieautoryzowanym komputerom dołączenie do sesji multiemisji i zapobiega modyfikowaniu transmisji przez atakujących.  

-   **Monitor dla punktów dystrybucji obsługujących multiemisję nieautoryzowanych**  
...
-   **Podczas eksportowania sekwencji zadań do lokalizacji sieciowej, należy zabezpieczyć tę lokalizację i kanał sieciowy**  
...
-   **Kanał komunikacyjny używany podczas przekazywania wirtualnego dysku twardego do Menedżera maszyny wirtualnej.**  

     Aby zapobiec modyfikowaniu danych podczas transferu za pośrednictwem sieci, należy użyć zabezpieczeń protokołu internetowego (IPsec) lub blok komunikatów serwera (SMB) między komputerem z konsoli programu Configuration Manager a komputerem z programem Virtual Machine Manager.  

-   **Jeśli należy użyć konta sekwencji zadań Uruchom jako, należy zastosować dodatkowe środki zabezpieczające**  
...
-   **Ogranicz i Monitoruj użytkowników administracyjnych, którym nadano rolę zabezpieczeń Menedżera wdrożenia systemu operacyjnego**  
     ...
     Użytkownicy administracyjni, którym nadano rolę zabezpieczeń Menedżera wdrożenia systemu operacyjnego mogą tworzyć certyfikaty z podpisem własnym, które można następnie używane do personifikowania klienta w celu uzyskania zasad klienta z programu Configuration Manager.  
...
-   Ujawnienie informacji i odmowa obsługi  
     ...
     Jeśli osoba atakująca może uzyskać kontrolę nad infrastrukturą programu Configuration Manager, może uruchomić sekwencje zadań uwzględniające formatowanie dysków twardych wszystkich komputerów klienckich. Sekwencje zdań można skonfigurować tak, aby zawierały poufne dane, takie jak na przykład informacje o kontach z uprawnieniami do dołączania domeny czy klucze licencji zbiorczej.  
...
-   Personifikacja i podniesienie uprawnień  
     ...
     Jeśli osoba atakująca uzyskuje certyfikat klienta, który jest używany dla rozruchowego nośnika sekwencji zadań i wdrożenia rozruchu środowiska PXE, ten certyfikat może służyć do podszycia się pod prawidłowego klienta programu Configuration Manager. W tym scenariuszu nieautoryzowany komputer może pobrać zasadę, która może zawierać dane poufne.  
...
-   Uwierzytelnianie klienta w punkcie migracji stanu jest realizowane za pośrednictwem tokenu programu Configuration Manager, który został wydany przez punkt zarządzania.  

     Ponadto programu Configuration Manager nie ograniczają lub zarządzanie ilość danych przechowywanych w punkcie migracji stanu i osoba atakująca może zapełnić dostępne miejsce na dysku i spowodować typu "odmowa usługi".  
...
##  <a name="BKMK_Privacy_HardwareInventory"></a>Informacje o ochronie prywatności dotyczące wdrażania systemu operacyjnego  
 ...
 Menedżer konfiguracji nie implementuje wdrożenia systemu operacyjnego domyślnie i przed gromadzenia informacji o stanie użytkownika lub tworzenia sekwencji zadań lub obrazów rozruchowych wymaga podjęcia kilku czynności konfiguracyjnych.  
