---
title: "Zarządzanie stanem użytkowników - programu Configuration Manager | Dokumentacja firmy Microsoft"
...
Użyj sekwencji zadań programu System Center Configuration Manager przechwycić i przywrócić dane stanu użytkownika w scenariuszach wdrażania systemu operacyjnego którym chcesz przechowywać stan użytkownika bieżącego systemu operacyjnego. Na przykład:  
...
3.  [Tworzenie sekwencji zadań w celu przechwycenia i przywrócenia stanu użytkownika w programie System Center Configuration Manager](../deploy-use/create-a-task-sequence-to-capture-and-restore-user-state.md). W szczególności należy dodać następujące kroki sekwencji zadań do przechwytywania danych użytkownika z komputera, przechowywania daty użytkownika w punkcie migracji stanu i przywracania danych użytkownika na komputerze:  

    -   [Zażądaj magazynu stanów](../understand/task-sequence-steps.md#BKMK_RequestStateStore) do żądania dostępu do punktu migracji stanu podczas przechwytywania stanu z komputera lub przywracania stanu komputera.  

    -   [Przechwyć stan użytkownika](../understand/task-sequence-steps.md#BKMK_CaptureUserState) do przechwytywania i przechowywania danych stanu użytkownika w punkcie migracji stanu.  

    -   [Przywróć stan użytkownika](../understand/task-sequence-steps.md#BKMK_RestoreUserState) do przywrócenia stanu użytkownika na komputerze docelowym, przywracając dane z punktu migracji stanu użytkownika.  
...
-   [Tworzenie sekwencji zadań w celu przechwycenia i przywrócenia stanu użytkownika](../deploy-use/create-a-task-sequence-to-capture-and-restore-user-state.md). W szczególności należy dodać następujące kroki sekwencji zadań w celu przechwycenia danych użytkownika z komputera i przywrócenia danych użytkownika na komputerze przy użyciu twardych linków.  
    ...
    -   [Przechwyć stan użytkownika](../understand/task-sequence-steps.md#BKMK_CaptureUserState) do przechwytywania i przechowywać dane o stanie użytkownika w lokalnym folderze przy użyciu twardych łączy.  

    -   [Przywróć stan użytkownika](../understand/task-sequence-steps.md#BKMK_RestoreUserState) do przywrócenia stanu użytkownika na komputerze docelowym, przywracając dane przy użyciu twardych łączy.  
...
-   Czy punkt migracji stanu odpowiada tylko na żądania przywrócenia danych stanu użytkownika. Włączenie tej opcji uniemożliwia użycie punktu migracji stanu do przechowywania danych o stanie użytkownika.  
 ...
 Aby uzyskać więcej informacji o punkcie migracji stanu i kroki, aby go skonfigurować, zobacz [punkt migracji stanu](prepare-site-system-roles-for-operating-system-deployments.md#BKMK_StateMigrationPoints).  

##  <a name="BKMK_ComputerAssociation"></a> Create a computer association  
 Utworzenie skojarzenia komputera powoduje zdefiniowanie relacji między komputerem źródłowym a komputerem docelowym podczas instalowania systemu operacyjnego na nowym sprzęcie i w przypadku konieczności przechwycenia i przywrócenia ustawień danych użytkownika. Komputer źródłowy jest istniejącego komputera zarządzanego przez program Configuration Manager. Po wdrożeniu nowego systemu operacyjnego na komputerze docelowym komputer źródłowy zawiera stan użytkownika, który został poddany migracji na komputer docelowy.  

> [!NOTE]  
>  Utwórz skojarzenie komputera między komputerów znajdujących się w lokacji nadrzędnej programu Configuration Manager z komputerów znajdujących się w lokacji podrzędnej nie jest obsługiwane. Skojarzenia komputerów są specyficzne dla lokacji i nie są replikowane.  
