...
description: "Pobierz wprowadzenie do analizy zasobów w programie System Center Configuration Manager."
ms.custom: na
ms.date: 2/22/2017
...
---
# <a name="introduction-to-asset-intelligence-in-system-center-configuration-manager"></a>Wprowadzenie do analizy zasobów programu System Center Configuration Manager

*Dotyczy: System Center Configuration Manager (bieżącej gałęzi)*

Inteligencji zasobów programu System Center Configuration Manager umożliwia spisu i zarządzanie wykorzystania licencji na oprogramowanie w całym przedsiębiorstwie za pomocą katalogu analizy zasobów. Wiele klas spisu sprzętu usługi instrumentacji zarządzania Windows (WMI) poszerza zakres zbieranych informacji dotyczących używanego sprzętu i oprogramowania. Ponad 60 raportów umożliwia zaprezentowanie tych informacji w formacie łatwym w użyciu. Wiele z tych raportów łączy się z bardziej szczegółowymi raportami, w których możesz znaleźć ogólne informacje, a następnie przejść do informacji bardziej szczegółowych. Możesz dodawać niestandardowe informacje do katalogu analizy zasobów, takie jak niestandardowe kategorie oprogramowania, rodziny oprogramowania, etykiety oprogramowania i wymagania dotyczące sprzętu. Możesz również nawiązać połączenie z usługą System Center Online, aby dynamicznie aktualizować katalog analizy zasobów przy użyciu najbardziej aktualnych dostępnych informacji. Klienci firmy Microsoft można uzgodnić wykorzystania licencji na oprogramowanie przedsiębiorstwa z licencjami na oprogramowanie zakupione, które są używane przez importowanie informacji o licencji oprogramowania do bazy danych lokacji programu Configuration Manager.  

##  <a name="BKMK_AssetIntelligenceCatalog"></a> Katalog analizy zasobów  

 Katalog inteligencji zasobów programu Configuration Manager to zestaw tabel bazy danych przechowywanych w bazie danych lokacji, które zawierają kategoryzacji i informacje identyfikacyjne o ponad 300 000 tytułach i wersjach oprogramowania. Te tabele bazy danych służą również do zarządzania wymaganiami sprzętowymi poszczególnych tytułów oprogramowania.  
...
##  <a name="BKMK_InventoriedSoftwareTitles"></a> Spisane tytuły oprogramowania  
 Informacje o spisanych tytułach oprogramowania można wyświetlić w obszarze roboczym **Zasoby i zgodność** w węźle **Spisane oprogramowanie** w węźle **Analiza zasobów** . Agent klienta spisu sprzętu zbiera informacje o dodawanych do spisu oprogramowania od klientów programu Configuration Manager oparte na tytuły oprogramowania, które są przechowywane w katalogu analizy zasobów.  

> [!WARNING]  
>  Agent klienta spisu sprzętu zbiera dane spisu w oparciu o klasy raportowania spisu sprzętu analizy zasobów włączone przez użytkownika. Aby uzyskać więcej informacji o sposobie włączania raportowania klasy, zobacz [Konfigurowanie analizy zasobów w programie System Center Configuration Manager](../../../../core/clients/manage/asset-intelligence/configuring-asset-intelligence.md).  

 Domyślnie dla poszczególnych spisanych tytułów oprogramowania są wyświetlane następujące informacje:  

-   **Nazwa**: Określa nazwę tytułu dodawanych do spisu oprogramowania.  

-   **Dostawca**: Określa nazwę dostawcy, która stworzyła tytuł dodawanych do spisu oprogramowania.  

-   **Wersja**: Określa wersję produktu tytułu dodawanych do spisu oprogramowania.  
...
-   **Label** [**1**, **2**, and **3**]: Określa etykiet niestandardowych, które są skojarzone z tytułu oprogramowania. Do spisanych tytułów oprogramowania można przypisać do trzech etykiet niestandardowych.  

-   **Liczba**: Określa liczbę klientów programu Configuration Manager, które mają spisanych tytułu oprogramowania.  

-   **Stan**: Określa stan sprawdzania poprawności tytułu dodawanych do spisu oprogramowania.  
 ...
 **Analizy zasobów** w węźle **zasobów i zgodność** obszaru roboczego jest stroną główną analizy zasobów w programie Configuration Manager. Na stronie głównej **Analiza zasobów** jest wyświetlany widok pulpitu nawigacyjnego podsumowania zawierający informacje o katalogu analizy zasobów.  
...
###  <a name="BKMK_SoftwareIdTagReports"></a> Raporty analizy zasobów dotyczące tagów identyfikacji oprogramowania  
 Raporty analizy zasobów dotyczące tagów identyfikacji oprogramowania zapewniają informacje o oprogramowaniu zawierającym tagi identyfikacji oprogramowania zgodne z normą ISO/IEC 19770-2. Tagi identyfikacji oprogramowania zapewniają autorytatywne informacje, które służą do identyfikowania zainstalowanego oprogramowania. Po włączeniu klasę raportowania spisu sprzętu SMS_SoftwareTag programu Configuration Manager gromadzi informacje o oprogramowaniu ze znacznikami identyfikacji oprogramowania. Następujące raporty zawierają informacje o oprogramowaniu:  

-   **Oprogramowanie 14A - Search for znacznikiem identyfikacji włączone oprogramowania**: Ten raport zawiera liczbę zainstalowanych programów z włączonym znacznikiem identyfikacji oprogramowania.  

-   **Oprogramowanie 14B - komputery z określonym znacznikiem identyfikacji włączone zainstalowane oprogramowanie**: Ten raport wyświetla wszystkie komputery, na których zainstalowane jest oprogramowanie z określonym znacznikiem identyfikacji włączone.  

-   **Zainstalowane oprogramowanie 14C - oprogramowanie identyfikacji tag włączane oprogramowania na określonym komputerze**: Ten raport wyświetla wszystkie zainstalowane oprogramowanie z określonym znacznikiem identyfikacji włączone na określonym komputerze.  
 ...
 **Zależności spisu sprzętu klienta**  
 Raporty oprogramowania zainstalowane analizy zasobów na podstawie danych zbieranych z klientów programu Configuration Manager przez rozszerzanie spisu sprzętu, aby włączyć raportowanie analizy zasobów. Ze względu na zależność tej raportowania spisu sprzętu raportów analizy zasobów są wyświetlane dane tylko z klientami programu Configuration Manager, które pomyślnie ukończyć procesy spis sprzętu z wymaganych klas raportowania WMI analizy zasobów włączone. Ponadto ponieważ klientów programu Configuration Manager wykonywać procesy spis sprzętu zgodnie z harmonogramem, zdefiniowane przez użytkownika administracyjnego, w danych raportów, które ma wpływ na dokładność raportów analizy zasobów mogą wystąpić opóźnienia. Na przykład spisany tytuł licencjonowanego oprogramowania może zostać odinstalowany po pomyślnym zakończeniu cyklu spisu sprzętu przez klienta. Jednak tytuł oprogramowania jest wyświetlany jako zainstalowane w raportów analizy zasobów do klienta dalej spisu sprzętu zaplanowane cykl raportowania.  

 **Zależności pakietów oprogramowania**  
 Ponieważ raportów analizy zasobów są oparte na danych tytułu zainstalowanego oprogramowania, które są zbierane za pomocą standardowych procesy spis sprzętu klienta programu Configuration Manager, niektóre dane tytuł oprogramowania nie może poprawnie zebrane. Na przykład instalacje oprogramowania, które nie są zgodne ze standardowymi procesami instalacji, lub instalacje oprogramowania, które zostały zmienione przed instalacją, mogą powodować niedokładne raportowanie analizy zasobów.  
...
>  Po rozwiązaniu konfliktu kategoryzacji element nie jest weryfikowany ponownie jako powodujący konflikt, jeśli w późniejszych aktualizacjach nie zostaną wprowadzone nowe informacje o elemencie.  
 ...
 Przykłady, kiedy stan sprawdzania poprawności może przejście z jednego stanu do innego, zobacz [zmian stanu sprawdzania poprawności przykład analizy zasobów w programie System Center Configuration Manager](../../../../core/clients/manage/asset-intelligence/example-validation-state-transitions-for-asset-intelligence.md).  
