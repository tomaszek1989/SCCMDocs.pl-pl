---
title: "Monitor i planu zarządzania energią | Dokumentacja firmy Microsoft"
...
---
# <a name="how-to-monitor-and-plan-for-power-management-in-system-center-configuration-manager"></a>Monitor i planu na potrzeby zarządzania zużyciem energii w programie System Center Configuration Manager

*Dotyczy: System Center Configuration Manager (bieżącej gałęzi)*

Użyj poniższe informacje ułatwią monitora i planu na potrzeby zarządzania zużyciem energii w programie System Center Configuration Manager.  

##  <a name="BKMK_How_to_use_reports"></a> Jak używać raportów do zarządzania energią  
 Zarządzanie energią w programie Configuration Manager obejmuje kilka raportów, aby ułatwić analizowanie ustawienia zasilania komputera i zużycie energii w organizacji. Te raporty mogą być pomocne podczas rozwiązywania problemów.  

 Aby móc korzystać z raportów dotyczących zarządzania energią, należy skonfigurować raportowanie dla swojej hierarchii. Aby uzyskać więcej informacji o raportowaniu w programie Configuration Manager, zobacz [raportowania w programie System Center Configuration Manager](../../../../core/servers/manage/reporting.md).  
...
## <a name="list-of-power-management-reports"></a>Lista raportów dotyczących zarządzania energią  
 Poniższa lista zawiera szczegóły raportów zarządzania energią, które są dostępne w programie Configuration Manager.  
...
|**Typ urządzenia**|Z listy rozwijanej wybierz typ komputera, dla którego chcesz utworzyć raport. Prawidłowe wartości to **wszystkie** (zarówno stacjonarnych i przenośnych komputerów), **pulpitu** (komputery stacjonarne tylko), i **komputera przenośnego** (tylko komputery przenośne).|  
...
|**Stan komputera**|Z listy rozwijanej wybierz stan komputera, dla którego ten raport ma zostać uruchomiony. Prawidłowe wartości to **wszystkie** (komputery, które zostały włączone lub wyłączone), **na** (komputery, które zostały włączone), i **poza** (komputery, które zostały włączone off w trybie uśpienia lub w hibernacji). Te wartości są zwracane tylko dla wybranego okresu raportowania.|  
|**Typ urządzenia**|Z listy rozwijanej wybierz typ komputera, dla którego chcesz utworzyć raport. Prawidłowe wartości to **wszystkie** (zarówno stacjonarnych i przenośnych komputerów), **pulpitu** (komputery stacjonarne tylko), i **komputera przenośnego** (tylko komputery przenośne). Te wartości są zwracane tylko dla wybranego okresu raportowania.|  
|**Możliwość uśpienia**|Z listy rozwijanej wybierz odpowiednią opcję, jeśli chcesz, aby w raporcie były wyświetlane komputery z możliwością uśpienia. Prawidłowe wartości to **wszystkie** (oba komputery możliwością i bez możliwości uśpienia), **nr** (komputerów będących bez możliwości uśpienia) i **tak** (komputery, które są w stanie uśpienia).|  
|**Możliwość wybudzenia z uśpienia**|Z listy rozwijanej wybierz odpowiednią opcję, jeśli chcesz, aby w raporcie były wyświetlane komputery z możliwością wybudzenia z uśpienia. Prawidłowe wartości to **wszystkie** (oba komputery możliwością i bez możliwości wznawiania ze stanu uśpienia), **nr** (komputerów, które są niezdolne do wznawiania ze stanu uśpienia), i **tak** (komputery, które są w stanie wznawiania ze stanu uśpienia).|  
|**Plan zasilania**|Z listy rozwijanej wybierz typy planów zasilania, które mają być wyświetlane w raporcie. Prawidłowe wartości to **wszystkie** (energii komputerów, które nie ma żadnych planów zarządzania stosowane, komputery, które jest stosowany jakiś plan zarządzania energią; komputery wykluczone z zarządzania energią), **nie określono** (komputerów, które nie jest stosowany jakiś plan zarządzania energią), **zdefiniowane** (komputerów, które jest stosowany jakiś plan zarządzania energią), i **wykluczone** (komputery, które zostały wykluczone z zarządzania zasilaniem).|  
...
|Nazwa raportu|Szczegóły|  
|-----------------|-------------|  
|**Aktywność komputera według komputerów**|Kliknij nazwę komputera, aby zobaczyć określonego działania na tym komputerze za pośrednictwem wybranego okresu raportowania. Działania te obejmują **komputera na** (komputerze włączono?), **monitorowania na** (monitor włączono?), i **użytkownika Active** (działanie wykryto z myszy, klawiatury lub Podłączanie pulpitu zdalnego).<br /><br /> Aby uzyskać więcej informacji, zobacz [Computer Activity by Computer Report](#BKMK_Comp_Activity_by_computer) w tym temacie.|  
 ...
 **Wykluczone komputery** raport wyświetla listę komputerów w określonej kolekcji, które zostały wykluczone z zarządzania energią programu Configuration Manager.  
...
> [!IMPORTANT]  
>  Jeśli komputer jest członkiem wielu kolekcji, gdzie każdy kolekcja ma planów zasilania różnych, najmniej restrykcyjne planu zasilania zostaną zastosowane.  
>   
>  Jeśli komputer jest członkiem wielu kolekcji, gdzie każdy kolekcja ma wznowienia różne czasy, najbliżej północy czasu zostanie użyta.  
...
-   Wykres przedstawiający całkowita miesięczne CO2 wygenerowanego (w tonach) dla komputerów w określonej kolekcji w określonym czasie.  

-   Wykres pokazujący średnią miesięcznych CO2 wygenerowanego (w tonach) dla każdego komputera w określonej kolekcji w określonym czasie.  

-   Tabela zawierająca Całkowity miesięczny CO2 generowane i średnia miesięczne CO2 generowany dla komputerów w określonej kolekcji w określonym przedziale czasu.  

 **Wpływ na środowisko** raport oblicza ilość CO2 wygenerowanego (w tonach) na podstawie czasu komputera lub monitor został włączony w okresie 24 godzin.  
...
-   Wykres przedstawiający całkowita codzienne CO2 wygenerowanego (w tonach) dla komputerów w określonej kolekcji w ciągu ostatnich 31 dni.  

-   Wykres przedstawiający średni dzienny CO2 wygenerowanego (w tonach) dla każdego komputera w określonej kolekcji w ciągu ostatnich 31 dni.  

-   Tabela zawierająca całkowita codzienne CO2 generowany i średni dzienny CO2 generatedfor komputerów w określonej kolekcji w ciągu ostatnich 31 dni.  

 **Dzienny wpływ** raport oblicza ilość CO2 wygenerowanego (w tonach) na podstawie czasu komputera lub monitor został włączony w okresie 24 godzin.  
...
|--------------------|-----------------|  
|**Kolekcja**|Z listy rozwijanej wybierz kolekcję dla tego raportu.|  
|**Wyświetl filtr**|Z listy rozwijanej wybierz **nieobsługiwane** Aby wyświetlić tylko komputery w określonej kolekcji, które są niezdolne do stanu uśpienia, hibernacji, wychodzi z trybu uśpienia lub wznawiania pracy ze stanu hibernacji. Wybierz **Pokaż wszystko** do wyświetlenia wszystkich komputerów w określonej kolekcji.|  
