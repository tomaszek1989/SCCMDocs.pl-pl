...
description: "Automatyczne wdrażanie aktualizacji oprogramowania, dodając nowe aktualizacje do grupy aktualizacji, który jest skojarzony z aktywnym wdrożeniem lub używając odsprzedawców ADR."
keywords: 
author: dougeby
ms.author: dougeby
manager: angrobe
...
*Dotyczy: System Center Configuration Manager (bieżącej gałęzi)*
 ...
 Można automatycznie wdrażać aktualizacje oprogramowania, dodając nowe aktualizacje oprogramowania do grupy aktualizacji skojarzone z aktywnym wdrożeniem lub można użyć reguły wdrażania automatycznego (ADR). Zazwyczaj użyje odsprzedawców ADR co miesiąc wdrażania aktualizacji oprogramowania (znany jako aktualizacje wtorek poprawek) oraz zarządzania aktualizacjami definicji. Jeśli potrzebujesz pomocy ustalenie wdrożenia, które jest odpowiednie dla Ciebie metody, zobacz [wdrażania aktualizacji oprogramowania](deploy-software-updates.md)
...
1.  W konsoli programu Configuration Manager, przejdź do **Biblioteka oprogramowania** > **Przegląd** > **aktualizacji oprogramowania**.  
...
> [!WARNING]  
>  Przed utworzeniem reguły ADR po raz pierwszy należy się upewnić, że ukończono synchronizację aktualizacji oprogramowania w lokacji. Jest to szczególnie ważne, podczas uruchamiania programu Configuration Manager z języka angielskiego, ponieważ klasyfikacji aktualizacji oprogramowania są wyświetlane w języku angielskim przed pierwszym synchronizacji i wyświetlony w języku lokalnym po ukończeniu synchronizacji aktualizacji oprogramowania. Reguły utworzone przed zsynchronizowaniem aktualizacji oprogramowania mogą nie działać prawidłowo po przeprowadzeniu synchronizacji, ponieważ ciąg tekstu może nie być identyczny.  
...
1.  W konsoli programu Configuration Manager, przejdź do **Biblioteka oprogramowania** **Przegląd** > **aktualizacji oprogramowania** > **zasady wdrażania automatycznego**.  
...
3.  Na stronie **Ogólne** skonfiguruj następujące ustawienia:  
    ...
    -   **Nazwa**: Określ nazwę ADR. Nazwa musi być unikatowa, opisywać cel reguły oraz umożliwiać jej identyfikację spośród innych użytkowników w lokacji programu Configuration Manager.  

    -   **Opis**: Określ opis dla ADR. Opis powinien zawierać omówienie reguły wdrażania i inne istotne informacje pomocne w identyfikacji reguły i odróżnienia jej między innymi w lokacji programu Configuration Manager. Pole opisu jest opcjonalne, jego limit długości wynosi 256 znaków i jest domyślnie puste.  
    ...
    -   **Włącz wdrożenie po uruchomieniu tej reguły**: Określ, czy włączyć wdrożenie aktualizacji oprogramowania po uruchomieniu ADR. W związku z tą specyfikacją należy wziąć pod uwagę następujące kwestie:  
...
6.  Na stronie Harmonogram szacowania określ, czy włączyć uruchamianie reguły ADR na podstawie harmonogramu. W przypadku włączenia tej opcji kliknij przycisk **Dostosuj** , aby skonfigurować harmonogram cykliczny.  
    ...
    > [!IMPORTANT]  
    >  Zostanie wyświetlony harmonogram synchronizacji punktu aktualizacji oprogramowania, aby ułatwić określenie częstotliwości harmonogramu szacowania. Nie należy konfigurować w harmonogramie szacowania częstotliwości większej niż wynikająca z harmonogramu synchronizacji aktualizacji oprogramowania. Czas rozpoczęcia harmonogramu jest oparty na czasie lokalnym komputera, na którym jest uruchomiona konsola programu Configuration Manager.  
...
8. Na stronie Czynności użytkownika skonfiguruj następujące ustawienia:  
    ...
    -   **Powiadomienia użytkowników**: Określ, czy wyświetlać powiadomienia o aktualizacjach oprogramowania w Centrum oprogramowania na komputerze klienckim skonfigurowanego **czas dostępności oprogramowania** oraz czy wyświetlać powiadomienia użytkowników na komputerach klienckich.  

    -   **Zachowanie ostatecznego terminu wdrożenia**: Określ zachowanie po osiągnięciu ostatecznego terminy wdrożenia aktualizacji oprogramowania. Określ, czy zainstalować aktualizacje oprogramowania we wdrożeniu oraz czy uruchomić ponownie system po zainstalowaniu aktualizacji oprogramowania niezależnie od skonfigurowanego okna obsługi. Aby uzyskać więcej informacji dotyczących okien obsługi, zobacz [sposobu używania okien obsługi](../../core/clients/manage/collections/use-maintenance-windows.md).  
...
10. Na stronie Ustawienia pobierania skonfiguruj następujące ustawienia:  
    ...
    -   **Zezwalaj klientom na współużytkowanie zawartości z innymi klientami w tej samej podsieci**: Określ, czy korzystanie z usługi BranchCache w odniesieniu do pobrań zawartości. Aby uzyskać więcej informacji na temat usługi BranchCache, zobacz [Pojęcia związane z zarządzaniem zawartością](../../core/plan-design/hierarchy/fundamental-concepts-for-content-management.md#branchcache).  
...
1.  W konsoli programu Configuration Manager, przejdź do **Biblioteka oprogramowania** > **Przegląd** > **aktualizacji oprogramowania** > **zasady wdrażania automatycznego**, a następnie wybierz odpowiednią regułę.  
