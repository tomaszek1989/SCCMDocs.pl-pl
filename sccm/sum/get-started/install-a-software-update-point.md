...
description: "Lokacje główne wymagają punktu aktualizacji oprogramowania w witrynie Administracja centralna dla oceny zgodności aktualizacji oprogramowania i aby wdrożyć oprogramowanie aktualizacje klientów."
keywords: 
author: dougeby
ms.author: dougeby
manager: angrobe
...
> [!IMPORTANT]  
>  Przed zainstalowaniem roli systemu lokacji punktu aktualizacji oprogramowania należy zweryfikować, że serwer spełnia wymagane zależności i ustala infrastrukturę punktu aktualizacji oprogramowania w danej lokacji. Aby uzyskać więcej informacji o sposobach planowania aktualizacji oprogramowania i ustalania infrastruktury punktu aktualizacji oprogramowania, zobacz [Plan aktualizacji oprogramowania](../plan-design/plan-for-software-updates.md).  

 Punkt aktualizacji oprogramowania jest wymagany w centralnej lokacji administracyjnej i w lokacjach głównych do włączenia oceny zgodności aktualizacji oprogramowania i wdrożenia aktualizacji oprogramowania na klientach. Punkt aktualizacji oprogramowania jest opcjonalny w lokacjach dodatkowych. Na serwerze, na którym są zainstalowane usługi WSUS, należy utworzyć rolę systemu lokacji punktu aktualizacji oprogramowania. Punkt aktualizacji oprogramowania prowadzi interakcję z usługami WSUS w celu skonfigurowania ustawień aktualizacji oprogramowania i żądania synchronizacji metadanych aktualizacji oprogramowania. Podczas hierarchii programu Configuration Manager należy najpierw zainstalować i skonfigurować punkt aktualizacji oprogramowania w centralnej lokacji administracyjnej, potem w podrzędnych lokacjach głównych, a następnie opcjonalnie w lokacjach dodatkowych. W przypadku autonomicznej lokacji głównej, a nie centralnej lokacji administracyjnej, należy najpierw zainstalować i skonfigurować punkt aktualizacji oprogramowania w lokacji głównej, a potem — opcjonalnie — w lokacjach dodatkowych. Niektóre ustawienia są dostępne tylko podczas konfigurowania punktu aktualizacji oprogramowania w lokacji najwyższego poziomu. Istnieją różne opcje, które należy wziąć pod uwagę zależnie od miejsca instalacji punktu aktualizacji oprogramowania.  
...
## <a name="proxy-server-settings"></a>Ustawienia serwera proxy  
 Ustawienia serwera proxy można skonfigurować na różnych stronach **lokacji Kreator tworzenia serwera systemu** lub **lokacji Kreator dodawania ról systemu** w zależności od wersji programu Configuration Manager można użyć.  
...
-   Należy skonfigurować serwer proxy, a następnie określić, kiedy serwer proxy ma być używany do aktualizacji oprogramowania. Skonfiguruj następujące ustawienia:  
    ...
    -   Określ, czy do korzystania z serwera proxy podczas synchronizowania Menedżera konfiguracji oprogramowania, aktualizacji i pobierania zawartości za pomocą zasady wdrażania automatycznego. Skonfiguruj ustawienia serwera proxy punktu aktualizacji oprogramowania na stronie **Ustawienia serwera proxy i konta** kreatora lub na karcie **Ustawienia serwera proxy i konta** właściwości punktu aktualizacji oprogramowania.  
...
### <a name="wsus-server-connection-account"></a>Konto połączenia serwera WSUS  
 Można skonfigurować konto, którego będzie używał serwer lokacji podczas łączenia się z usługami WSUS uruchamianymi w punkcie aktualizacji oprogramowania. Jeśli takie konto nie jest skonfigurowane, konto komputera serwera lokacji programu Configuration Manager korzysta z nawiązać połączenia z programem WSUS. Skonfiguruj ustawienia konta połączenia serwera WSUS na stronie **Ustawienia serwera proxy i konta** kreatora lub na karcie **Ustawienia serwera proxy i konta** właściwości punktu aktualizacji oprogramowania.  Konto można skonfigurować w różnych miejscach kreatora, zależnie od wersji programu Configuration Manager można użyć.  

 Aby uzyskać więcej informacji o kontach programu Configuration Manager, zobacz [kont używanych w programie System Center Configuration Manager](../../core/plan-design/hierarchy/accounts.md).  
...
## <a name="synchronization-schedule"></a>Harmonogram synchronizacji  
 ...
 Po włączeniu harmonogramu można skonfigurować cykliczny — prosty lub niestandardowy — harmonogram synchronizacji. Podczas konfigurowania harmonogramu prostego czas rozpoczęcia jest oparty na czas lokalny dla komputera, na którym jest uruchomiona konsola programu Configuration Manager w momencie tworzenia harmonogramu. Podczas konfigurowania harmonogramu niestandardowego czas rozpoczęcia opiera się na czas lokalny dla komputera, na którym jest uruchomiona konsola programu Configuration Manager.  
...
Po zainstalowaniu punktu aktualizacji oprogramowania, począwszy od najbardziej na wierzchu lokacji w hierarchii programu Configuration Manager. Powtórz procedury opisane w tym temacie, aby zainstalować punkt aktualizacji oprogramowania w lokacjach podrzędnych.

Po określeniu oprogramowanie zainstalowane punkty aktualizacji, przejdź do [synchronizowanie aktualizacji oprogramowania](synchronize-software-updates.md).
